...
import { NeneDesktopService } from "../services/nene-desktop-service.js";
import { NeneWebClient } from "../services/nene-web-client.js";
...
export interface ControllerContainer {
  ...
  neneDesktopService: NeneDesktopService;
...
export async function createContainer(): Promise<ControllerContainer> {
  ...
  const neneWebClient = new NeneWebClient({
    baseUrl: env.neneWebBaseUrl,
    desktopAppId: env.neneDesktopAppId,
    updateChannel: env.neneUpdateChannel,
  });
  const neneDesktopService = new NeneDesktopService(configStore, neneWebClient);
