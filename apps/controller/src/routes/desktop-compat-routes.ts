...
import {
  ...
  neneDesktopStatusResponseSchema,
} from "@nexu/shared";
...
export function registerDesktopCompatRoutes(
  ...
  app.openapi(
    createRoute({
      method: "get",
      path: "/api/internal/desktop/nene-status",
      tags: ["Desktop"],
      responses: {
        200: {
          content: {
            "application/json": { schema: neneDesktopStatusResponseSchema },
          },
          description: "nene desktop integration status",
        },
      },
    }),
    async (c) => c.json(await container.neneDesktopService.getStatus(), 200),
  );
