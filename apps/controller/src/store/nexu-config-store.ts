...
import {
  ...
  neneDesktopPersistedStateSchema,
...
type NeneDesktopPersistedState = z.infer<
...
function readDesktopNeneState(config: NexuConfig): NeneDesktopPersistedState {
  const desktop = config.desktop as Record<string, unknown>;
  return neneDesktopPersistedStateSchema.parse(desktop.nene ?? {});
}
...
export class NexuConfigStore {
  ...
  async ensureNeneDesktopState(): Promise<NeneDesktopPersistedState> {
    const config = await this.getConfig();
    const nextState = readDesktopNeneState(config);
    const desktop = config.desktop as Record<string, unknown>;
    const currentSerialized = JSON.stringify(desktop.nene ?? null);
  ...
  async getPersistedNeneDesktopState(): Promise<NeneDesktopPersistedState> {
    return this.ensureNeneDesktopState();
  }
