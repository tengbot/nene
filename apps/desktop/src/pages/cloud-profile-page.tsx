...
type NeneDesktopStatus = Awaited<ReturnType<typeof getNeneDesktopStatus>>;

function getNeneConnectionLabel(
  status: NeneDesktopStatus["connectionStatus"] | undefined,
): string {
  switch (status) {
    case "configured":
      return "Configured";
...
function getNeneConnectionTone(
  status: NeneDesktopStatus["connectionStatus"] | undefined,
): "default" | "loading" | "connected" | "error" {
  switch (status) {
    case "configured":
      return "loading";
    case "connected":
...
export function CloudProfilePage() {
  ...
  const [neneStatus, setNeneStatus] = useState<NeneDesktopStatus | null>(null);
  ...
  const refreshCloudStatus = useCallback(
    async (options?: {
      ...
      try {
        const [status, nextNeneStatus] = await Promise.all([
          getDesktopCloudStatus(),
          getNeneDesktopStatus(),
        ]);
        setCloudStatus(status);
        setNeneStatus(nextNeneStatus);
  ...
  useEffect(() => {
    void refreshCloudStatus({ silent: true, loading: true });
  }, [refreshCloudStatus]);
  ...
  const neneModeLabel =
    neneStatus?.mode === "nene-account" ? "Nene Account Mode" : "Local Mode";
  const neneConnectionLabel = getNeneConnectionLabel(
    neneStatus?.connectionStatus,
  );
  const neneConnectionTone = getNeneConnectionTone(
    neneStatus?.connectionStatus,
  );
  ...
  return (
    <div className="runtime-page cloud-profile-page">
      <header className="runtime-header cloud-profile-header">
        <div>
          <span className="runtime-eyebrow">Nene Account</span>
          <h1>Local Mode and account adapter</h1>
          <p>
            Phase 1 keeps upstream cloud profiles for compatibility and layers
            in a thin nene-web adapter for future account mode. Runtime still
            stays local in both modes.
          </p>
        </div>
      </header>

      <section className="cloud-profile-status-card">
        <span
          className={
            neneConnectionTone === "connected"
              ? "cloud-profile-status-dot is-connected"
              : neneConnectionTone === "loading"
                ? "cloud-profile-status-dot is-loading"
                : neneConnectionTone === "error"
                  ? "cloud-profile-status-dot is-error"
                  : "cloud-profile-status-dot"
          }
        />
        <div>
          <strong>
            {neneModeLabel} · {neneConnectionLabel}
          </strong>
          <p>
            {neneStatus?.configured
              ? "nene-web adapter is configured. Entitlements and release checks can be layered in without moving runtime execution to the cloud."
              : "No nene-web endpoint is configured yet. Local Mode remains fully usable with BYOK and local runtime."}
          </p>
        </div>
      </section>
      ...
      <section className="cloud-profile-card">
        <div className="cloud-profile-section-head">
          <strong>nene-web Adapter</strong>
          <span>
            These values are the new public integration seam. The legacy cloud
            profile model stays in place underneath for upstream compatibility.
          </span>
        </div>
        <dl className="cloud-profile-endpoints">
          <div>
            <dt>Mode</dt>
            <dd>{neneModeLabel}</dd>
          </div>
          <div>
            <dt>Status</dt>
            <dd>{neneConnectionLabel}</dd>
          </div>
          <div>
            <dt>Web Base URL</dt>
            <dd>{neneStatus?.webBaseUrl ?? "Not configured"}</dd>
          </div>
          <div>
            <dt>Desktop App ID</dt>
            <dd>{neneStatus?.desktopAppId ?? "Not configured"}</dd>
          </div>
          <div>
            <dt>Update Channel</dt>
            <dd>{neneStatus?.updateChannel ?? "Not configured"}</dd>
          </div>
          <div>
            <dt>Cached Entitlements</dt>
            <dd>
              {neneStatus && neneStatus.entitlements.length > 0
                ? `${neneStatus.entitlements.length} cached entitlement(s)`
                : "No cached entitlements yet"}
            </dd>
          </div>
        </dl>
      </section>
